...
pub static DEFAULT_SHADER: LazyLock<Shader> = LazyLock::new(|| Shader::default());

pub static SPRITE_SHADER: LazyLock<Shader> = LazyLock::new(|| {
    let (vert, frag, id);
    unsafe {
...
pub static BATCH_SHADER: LazyLock<Shader> = LazyLock::new(|| {
    let (vert, frag, id);
    unsafe {
        let (v, f, i) = Shader::do_shader(
...
pub static NO_TEXTURE_SHADER: LazyLock<Shader> = LazyLock::new(|| {
    let (vert, frag, id);
    unsafe {
        let (v, f, i) = Shader::do_shader(
...
impl Shader {
    ...
    pub unsafe fn new(vert: &str, frag: &str) -> Result<Shader, io::Error> {
